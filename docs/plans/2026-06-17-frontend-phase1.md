# Frontend Fase 1 — `synapse ui` + dashboard reale (stato rete + chat) — Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development. Steps use checkbox (`- [ ]`).

**Goal:** Ogni nodo lancia `synapse ui`: un server di controllo locale che serve un frontend reale (ricostruito dal mock) con cui **vedere lo stato della rete** (#2) e **fare inferenza via chat** (#3), parlando solo col server locale (che fa da proxy al coordinator, niente CORS).

**Architecture:** `synapse ui --coordinator <url> --port 8500` avvia una FastAPI che (a) serve `synapse/ui/static/index.html`, (b) espone `/api/config`, `/api/registry` (proxy GET coordinator `/registry`), `/api/chat` (proxy POST coordinator `/v1/chat/completions`). Il frontend (React via CDN, single file, fedele al mock) fa polling di `/api/registry`, calcola la coverage lato client, e chatta via `/api/chat`. Crea/aggiungi-rete (#1) e MCP (#4) sono fasi successive sullo stesso server.

**Tech Stack:** Python · FastAPI + httpx (proxy) · React 18 + Babel standalone via CDN (frontend single-file, nessun build) · IBM Plex (Google Fonts). Riferimento visivo: `frontend/_mock/Synapse Dashboard.dc.html`.
...
## File Structure
```
synapse/ui/__init__.py          # NUOVO (vuoto)
synapse/ui/server.py            # NUOVO: create_ui_app(coordinator_url) + endpoint /api/*
synapse/ui/static/index.html    # NUOVO: frontend reale (Rete + Chat)
synapse/cli.py                  # MOD: comando `ui`
...
## Task 1: `synapse/ui/server.py` (control server + proxy)

**Files:** create `synapse/ui/__init__.py` (empty), `synapse/ui/server.py`; create `tests/test_ui_server.py`. (index.html viene creato nel Task 2; per ora il test del serve lo salta o crea un file minimo — vedi Step 1.)
...
- [ ] **Step 2: run FAIL** — `/Users/alberto/Projects/AI/synapse/.venv/bin/python -m pytest tests/test_ui_server.py -v` → ImportError.

- [ ] **Step 3: crea `synapse/ui/__init__.py`** (vuoto) e `synapse/ui/server.py`:
```python
import os
...
- [ ] **Step 4: run PASS** — `/Users/alberto/Projects/AI/synapse/.venv/bin/python -m pytest tests/test_ui_server.py -v` → 2 fast pass; `... -m slow -v` → proxy test pass. (Il `test_serves_index_html` passa col fallback minimo finché non c'è il vero index.html.)

- [ ] **Step 5: commit**
```bash
cd /Users/alberto/Projects/AI/synapse && git add synapse/ui/__init__.py synapse/ui/server.py tests/test_ui_server.py && git commit -m "feat(ui): control server locale con proxy /api/registry e /api/chat"
```

---

## Task 2: frontend reale `synapse/ui/static/index.html`

**Files:** create `synapse/ui/static/index.html`.

> Ricostruisci il design del mock `frontend/_mock/Synapse Dashboard.dc.html` come app reale **single-file** (React 18 + Babel standalone via CDN, nessun build). Niente runtime Claude Design. Il frontend parla SOLO col server locale (`/api/*`, stessa origin).

- [ ] **Step 1: implementa `synapse/ui/static/index.html`** con questi requisiti precisi:
...
`cd /Users/alberto/Projects/AI/synapse && .venv/bin/python -c "import re,sys; h=open('synapse/ui/static/index.html').read(); assert 'react' in h.lower() and 'Assemblaggio' in h and '/api/registry' in h and '/api/chat' in h; print('index ok', len(h), 'bytes')"`

- [ ] **Step 4: commit**
```bash
cd /Users/alberto/Projects/AI/synapse && git add synapse/ui/static/index.html && git commit -m "feat(ui): frontend reale (vista Rete + Chat) fedele al mock"
```

---

## Task 3: comando CLI `synapse ui` + docs

**Files:** modify `synapse/cli.py`; create `docs/examples/frontend.md`; modify `.gitignore`, `README.md`.

- [ ] **Step 1: aggiungi il comando `ui` in `synapse/cli.py`** (dopo `coordinator`):
```python
@app.command()
def ui(
...
Il browser parla solo col server locale `synapse ui`, che fa da proxy al coordinator (niente CORS).

In arrivo: creare/aggiungersi a una rete dal frontend (Fase 2) e configurare tool MCP (Fase 3).
```

- [ ] **Step 5:** aggiungi al `README.md` (sezione Quickstart) una riga: "**Frontend:** `synapse ui --coordinator http://IP:9000` → dashboard rete + chat (vedi [docs/examples/frontend.md](docs/examples/frontend.md))."

- [ ] **Step 6: reinstalla + verifica il comando**
`cd /Users/alberto/Projects/AI/synapse && .venv/bin/pip install -e . >/dev/null 2>&1 && .venv/bin/synapse --help | grep -q "ui" && echo "comando ui ok"`

- [ ] **Step 7: suite completa** — `... pytest -q -p no:warnings` → verde.

- [ ] **Step 8: commit**
```bash
cd /Users/alberto/Projects/AI/synapse && git add synapse/cli.py pyproject.toml .gitignore docs/examples/frontend.md README.md && git commit -m "feat(cli): comando 'synapse ui' (frontend locale) + docs"
```
...
**Coverage:** #2 stato rete (vista Rete: assemblaggio + coverage + nodi + grafo, Task 2) ✓; #3 chat (vista Chat via /api/chat, Task 2) ✓; server di controllo locale che fa da base per #1/#4 (Task 1) ✓; comando `synapse ui` (Task 3) ✓. #1 crea/aggiungi e #4 MCP esplicitamente fasi 2/3.
