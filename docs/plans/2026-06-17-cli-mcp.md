...
**Goal:** Configure and use **MCP tools from the command line**: `axyn mcp` (add/list/remove of MCP servers, persisted) and `axyn infer --mcp` (queries the model, running the tool-calling loop with the configured MCP tools). This way an AI with shell access (e.g. Claude Code) can also do #4 via the CLI.

**Architecture:** MCP config persisted in `~/.axyn/mcp.json` (override with `AXYN_HOME`). The `mcp` command (single word + switches, consistent with `model --info`) manages the config and, for `--list`, discovers the tools via `McpRegistry`. `infer --mcp` builds an `McpRegistry` from the config, fetches the tools, and runs `run_tool_loop` against the `/v1/chat/completions` endpoint of the coordinator or a peer (both expose it), executing the tools via MCP. Reuses everything that already exists (`McpRegistry`, `run_tool_loop`).

**Tech Stack:** Python · Typer · the existing `axyn/ui/{mcp,agent}.py` · httpx.
...
## File Structure
```
axyn/mcp_config.py           # NEW: load/save MCP servers (~/.axyn/mcp.json)
axyn/cli.py                  # MOD: `mcp` command + `infer --mcp`
...
- [ ] **Step 3: implement `axyn/mcp_config.py`**
```python
import json
import os
...
- [ ] **Step 5: commit**
```bash
cd /Users/alberto/Projects/AI/axyn && git add axyn/mcp_config.py tests/test_mcp_config.py && git commit -m "feat(cli): mcp_config (persist MCP servers in ~/.axyn/mcp.json)"
```

---

## Task 2: `axyn mcp` command
...
- [ ] **Step 3: add the command in `axyn/cli.py`** (after `schema`, or near the other commands):
```python
@app.command()
def mcp(
...
- [ ] **Step 4: run PASS** — `... pytest tests/test_cli_mcp.py -v` (fast) and `-m slow -v` (discovery with the echo server) → PASS. Verify `cd /Users/alberto/Projects/AI/axyn && .venv/bin/axyn --help | grep -q mcp`.

- [ ] **Step 5: commit**
```bash
cd /Users/alberto/Projects/AI/axyn && git add axyn/cli.py tests/test_cli_mcp.py && git commit -m "feat(cli): 'axyn mcp' command (add/list/remove MCP servers)"
```
...
- [ ] **Step 3: extend `infer` in `axyn/cli.py`** — add the `--mcp` option to the signature:
```python
...
- [ ] **Step 5: commit**
```bash
cd /Users/alberto/Projects/AI/axyn && git add axyn/cli.py tests/test_cli_mcp.py && git commit -m "feat(cli): infer --mcp (tool-calling loop with the configured MCP tools)"
```
...
axyn mcp --remove fs
```
The config is saved in `~/.axyn/mcp.json` (override with `AXYN_HOME`). `--mcp` requires `--coordinator` or `--peer` (both expose `/v1`). It requires a model that supports tool-calling.
```
...
- [ ] **Step 3: commit**
```bash
cd /Users/alberto/Projects/AI/axyn && git add docs/examples/agents.md && git commit -m "docs: MCP tools from the CLI (axyn mcp + infer --mcp)"
```
...
**Coverage (#4 via CLI):** persisted config (Task 1) ✓; `axyn mcp` command add/list/remove (Task 2) ✓; `infer --mcp` tool-calling loop against coordinator/peer `/v1` (Task 3) ✓; docs (Task 4) ✓. Reuses `McpRegistry`/`run_tool_loop`. Consistent with the "single word + switches" principle (`mcp --add/--list/--remove`).

**Placeholder scan:** no TODO; code complete.

**Type consistency:** `mcp_config.{load_servers/save_servers/add_server/remove_server}`; `mcp` command with `--add/--command/--args/--remove`; `infer --mcp` uses `load_servers` + `McpRegistry` + `run_tool_loop`, posts to `{target}/v1/chat/completions`. `AXYN_HOME` for the tests.
```
