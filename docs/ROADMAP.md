...
- [x] **API OpenAI-compatibile** (`/v1/chat/completions` + `/v1/models`) sul coordinator: chat template + sampling (temperature/top_p/repetition_penalty/seed) — [piano](./plans/2026-06-17-openai-api.md) · [guida agenti](./examples/agents.md). Collega client/agenti OpenAI.
- [x] **Stop all'EOS + tool/function calling** (`tools`/`tool_calls`) — [piano](./plans/2026-06-17-tool-calling.md). Base per agenti MCP (i tool li esegue l'host; il modello decide). *(streaming SSE + Anthropic/LiteLLM per Claude Code = prossimi passi)*
