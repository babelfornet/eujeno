# Axyn node image (linux/arm64) — runs `axyn serve` for a slice of layers.
...
# Entrypoint = axyn CLI; the arguments (serve --coordinator ... --stages ...)
# are passed via `docker run ... axyn-node serve ...`.
ENTRYPOINT ["axyn"]
