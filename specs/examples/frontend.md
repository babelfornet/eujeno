...
The browser talks **only** to the local `eujeno ui` server, which proxies to the coordinator (no CORS issues).
...
> Security: `eujeno ui` listens on `127.0.0.1` and starts processes on **your** machine (`python -m eujeno coordinator|serve`). Use it only locally / in a trusted environment.

## MCP tools ("MCP" tab)

`eujeno ui` acts as an **MCP host**: from the **MCP** tab you configure MCP servers (stdio) and their tools become usable by the model.
...
- **Enable "use MCP tools"** (toggle): in chat, requests pass the tools to the model; when the model calls a tool, `eujeno ui` **runs** it on the MCP server and sends back the result (tool-calling loop). Below the response you see which tools were used (`🔧 name → result`).
