...
| `eujeno up --model <id> [--dtype bfloat16] [--device <dev>]` | One-command bring-up: starts a coordinator + a node that covers all layers. **Auto-selects the GPU** (Apple `mps` / CUDA) on this machine when present, else CPU; override with `--device cpu`. The dtype then defaults to **bfloat16 on GPU, float32 on CPU** (override with `--dtype`). `--dry-run` prints the commands without starting them. |
| `eujeno serve --stages "<spec>" ...` | Starts a node that hosts certain blocks. `--device <cpu\|cuda\|mps\|auto>` picks where that node's layers run (default **auto**: the GPU if present, else CPU — interoperates with CPU peers); `--dtype` defaults to **bfloat16 on GPU, float32 on CPU**. |
